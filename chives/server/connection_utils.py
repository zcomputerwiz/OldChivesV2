...
async def send_all_first_reply(
    func: str, arg: Any, peers: List[WSChivesConnection], timeout=15
) -> Optional[Tuple[Any, WSChivesConnection]]:
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""

    async def do_func(peer_x: WSChivesConnection, func_x: str, arg_x: Any):
...
<<<<<<< HEAD:chives/server/connection_utils.py
async def send_to_random(func: str, arg: Any, peers: List[WSChivesConnection]) -> Optional[Tuple[Any, WSChivesConnection]]:
=======
async def send_to_random(
    func: str, arg: Any, peers: List[WSChivesConnection]
) -> Optional[Tuple[Any, WSChivesConnection]]:
>>>>>>> upstream/main:chives/server/connection_utils.py
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""
