...
import chives.cmds.configure as chives_configure
from chives import __version__
from chives.cmds.chives import monkey_patch_click
from chives.cmds.init_funcs import init
from chives.seeder.util.config import patch_default_seeder_config
from chives.seeder.util.service_groups import all_groups, services_for_groups
from chives.seeder.util.service import launch_service, kill_service
...
@click.group(
    help=f"\n  Manage the Chives Seeder ({__version__})\n",
    epilog="Try 'chives seeder start crawler' or 'chives seeder start server'",
...
@cli.command("version", short_help="Show the Chives Seeder version")
...
def init_cmd(ctx: click.Context, **kwargs):
    print("Calling Chives Seeder Init...")
    init(None, ctx.obj["root_path"], True)
    if os.environ.get("CHIVES_ROOT", None) is not None:
        print(f"warning, your CHIVES_ROOT is set to {os.environ['CHIVES_ROOT']}.")
    root_path = ctx.obj["root_path"]
    print(f"Chives directory {root_path}")
    if root_path.is_dir() and not Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIVES_ROOT is set, but there is no config
        # This really shouldn't happen, but if we dont have the base chives config, we can't continue
...
def configure(
    ...
    chives_configure.configure(root_path, "", "", "", "", "", "", "", "", testnet, "")

    config: Dict = load_config(DEFAULT_ROOT_PATH, "config.yaml")
    change_made = False
    if testnet is not None:
        if testnet == "true" or testnet == "t":
            print("Updating Chives Seeder to testnet settings")
            port = 58444
            network = "testnet7"
            bootstrap = ["testnet-node.chives.net"]
            ...
            print("Updating Chives Seeder to mainnet settings")
            port = 8444
            network = "mainnet"
            bootstrap = ["node.chives.net"]
    ...
    if change_made:
        print("Restart any running Chives Seeder services for changes to take effect")
